...
import models
from self_play import EvoNode, SelfPlay
...
class DiagnoseModel:
    ...
    def __init__(self, checkpoint, config):
        ...
        self.controller.load_state_dict(checkpoint["controller_weights"]) # initially determined in muzero.py

    def use_controller_with_muzero(self, observation):
        observation = (
        ...
        return initial_node, initial_hidden_state
    ...
    def get_virtual_trajectory_from_obs(
        ...
        initial_node, encoded_state = self.use_controller_with_muzero(observation)
        trajectory_info.store_info(initial_node, None, numpy.NaN)

        virtual_to_play = to_play
        for i in range(horizon):
            action = torch.argmax(self.controller(encoded_state).flatten())
            ...
            value, reward, policy_logits, hidden_state = self.model.recurrent_inference(
                initial_node.state,
                torch.tensor([[action]]).to(initial_node.state.device),
            )
            ...
            trajectory_info.store_info(
                initial_node, action, reward, new_prior_root_value=value
            )
    ...
    def compare_virtual_with_real_trajectories(
        ...
        initial_node, _ = self.use_controller_with_muzero(first_obs)
        self.plot_mcts(initial_node, plot)
        real_trajectory_info.store_info(initial_node, None, numpy.NaN)
        for i, action in enumerate(virtual_trajectory_info.action_history):
            ...
            initial_node, _ = self.use_controller_with_muzero(observation)
            real_trajectory_info.store_info(initial_node, action, reward)
            if done:
    ...
    def plot_mcts(self, root, plot=True):
        ...
        # print(graph.source)
        graph.render("NES", view=plot, cleanup=True, format="pdf")
        return graph
...
class Trajectoryinfo:
    ...
    def store_info(self, root, action, reward, new_prior_root_value=None):
        ...
        self.prior_policies.append(
            [
                root.children[action].nes_prob_logit
        ...
        self.root_value_after_planning.append(root.mean_value())
