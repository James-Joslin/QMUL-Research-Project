...
class SimpleNet(nn.Module): # Controller model and some utility functions that make it easier to use with the NES optimiser
    def __init__(self, input_size=0, layer_size=0, hidden_layers=0, output_size=0):
        super(SimpleNet, self).__init__()
        self.hidden_layers = hidden_layers
        self.input_layer = nn.Linear(input_size, layer_size) # Input (encoded hidden state) -> Hidden, because input is a flat vector, it won't work with conv outputs from muzero's resnet just fully-connected
        if hidden_layers > 0: # the parameter hidden layers refers to additional layers
            self.hidden_layers_list = nn.ModuleList()
            for _ in range(hidden_layers):
                self.hidden_layers_list.append(nn.Linear(layer_size, layer_size))
        self.output_layer = nn.Linear(layer_size, output_size) # Hidden -> Out (actions)

    def forward(self, x):
        x = torch.relu(self.input_layer(x)) # ReLU activation on forward parse
        if self.hidden_layers > 0: # Go through additional hidden layers if there are any
            for hidden_layer in self.hidden_layers_list:
                x = torch.relu(hidden_layer(x))
        x = torch.softmax(self.output_layer(x), dim=-1) # Output policy logits for action options as a flattened vector with softmax activation
        return x
    
    def get_nn_parameters(self):
        return torch.cat([param.view(-1) for param in self.parameters()]) # Convert parameters to a flat tensor
    
    def set_nn_parameters(self, flat_parameters):
        idx = 0 # copies a parameter set into the controller model
