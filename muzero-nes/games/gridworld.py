...
class MuZeroConfig:
    def __init__(self):
        ...
        # nes parameters
        self.num_es_workers = 4 # number of paralllel fitness workers used by the nes optimiser in controller_trainer.py
        self.num_layers_es = 1 # hidden layers
        self.layer_size_es = 32 # Number of nodes per layer
        
        self.population_size_es = 120 # number of parameter sets to test (after perturbation)
        self.noise_std_dev_es = 0.1 # perturbation noise standard deviation
        self.learning_rate_es = 0.1 # gradient descent learning rate
        self.generation_es = 1 # Generations per parse
        self.num_actions_es = 15 # trajectory length
        ...
        ### Training
        self.results_path = pathlib.Path(__file__).resolve().parents[1] / "results" / pathlib.Path(__file__).stem / datetime.datetime.now().strftime("%Y-%m-%d--%H-%M-%S-%f")  # Path to store the model weights and TensorBoard logs
        ...
        self.controller_delay = 25 # Number of seconds to wait between games played by the optimiser - limits the number of optimisation parses that can occur during the training cycle of MuZero
        self.self_play_delay = 10  # Number of seconds to wait after each played game
        self.training_delay = 0  # Number of seconds to wait after each training step
        self.ratio = None  # Desired training steps per self played step ratio. Equivalent to a synchronous version, training can take much longer. Set it to None to disable it
        # fmt: on

    def visit_softmax_temperature_fn(self, trained_steps): # not used by NES
        """
        ...
        Returns:
            Positive float.
        """
        return 0.35 # this function is not used by NES
