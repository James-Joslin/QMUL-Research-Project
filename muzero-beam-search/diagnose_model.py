...
import models
from self_play import BeamSearch, BeamNode, SelfPlay
...
class DiagnoseModel:
    ...
    def get_virtual_trajectory_from_obs(
        ...
        trajectory_info.store_info(root, None, numpy.NaN)
        ...
        for i in range(horizon):
            ...
            trajectory_info.store_info(
                root, action, reward, new_prior_root_value=value
            )
    ...
    def compare_virtual_with_real_trajectories(
        ...
        real_trajectory_info.store_info(root, None, numpy.NaN)
        for i, action in enumerate(virtual_trajectory_info.action_history):
            ...
            real_trajectory_info.store_info(root, action, reward)
            if done:
