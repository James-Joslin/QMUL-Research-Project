...
class MuZeroConfig:
    ...
    def visit_softmax_temperature_fn(self, trained_steps):
        ...
        # return self.exp_deacy_curve(trained_step, 0.0005)
        
        # # Sigmoid based curve
        # return self.accelerating_curve(trained_steps, 10000)

    def exp_deacy_curve(self, step, decay_rate):
        return 1 * numpy.exp(-decay_rate * step)
    ...
    def sigmoid(self, x, k):
        ...
        Parameters:
            ...
            k: Scaling factor for the sigmoid function.

        Returns:
            A float between 0 and 1.
        """
        return 1 / (1 + numpy.exp(-k * x))

    def accelerating_curve(self, x, max_x):
        """
        Returns a value between 0 and 1 that represents the point on an accelerating curve,
        where the curve starts slowly decreasing and accelerates towards 0 as x approaches max_x.

        Parameters:
            x: The current position on the x-axis (0 <= x <= max_x).
            max_x: The maximum value of x.

        Returns:
            A float between 0 and 1.
        """
        return 1 - self.sigmoid(x / max_x - .55, 10)
